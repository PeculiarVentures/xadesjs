...
import { CRLValues } from "./crl_value";
import { OCSPValues } from "./ocsp_value";
import { OtherValues } from "./other_value";
...
export class RevocationValues extends XmlXadesObject {
    ...
    protected name = XmlXades.ElementNames.RevocationValues;
    ...
    public LoadXml(element: Element): void {
        ...
        let xml = this.GetElement(XmlXades.ElementNames.CRLValues, false);
        if (xml) {
            this.CRLValues = new CRLValues();
            this.CRLValues.LoadXml(xml);
        }

        xml = this.GetElement(XmlXades.ElementNames.OCSPValues, false);
        if (xml) {
            this.OCSPValues = new OCSPValues();
            this.OCSPValues.LoadXml(xml);
        }

        xml = this.GetElement(XmlXades.ElementNames.OtherValues, false);
        if (xml) {
            this.OtherValues = new OtherValues();
