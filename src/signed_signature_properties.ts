...
export class SignedSignatureProperties extends XmlXadesObject {
    ...
    protected name = XmlXades.ElementNames.SignedSignatureProperties;
    ...
    public LoadXml(element: Element): void {
        ...
        let xml = this.GetElement(XmlXades.ElementNames.SigningTime, false);
        if (xml) {
            this.signingTime = XmlJs.Convert.ToDateTime(xml.textContent!);
        }

        xml = this.GetElement(XmlXades.ElementNames.SigningCertificate, false);
        if (xml) {
            this.signingCertificate = new SigningCertificate();
            this.signingCertificate.LoadXml(xml);
        }

        xml = this.GetElement(XmlXades.ElementNames.SignaturePolicyIdentifier, false);
        if (xml) {
            this.signaturePolicyIdentifier = new SignaturePolicyIdentifier();
            this.signaturePolicyIdentifier.LoadXml(xml);
        }

        xml = this.GetElement(XmlXades.ElementNames.SignatureProductionPlace, false);
        if (xml) {
            this.signatureProductionPlace = new SignatureProductionPlace();
        ...
        xml = this.GetElement(XmlXades.ElementNames.SignerRole, false);
        if (xml) {
            this.signerRole = new SignerRole();
    ...
    public GetXml(): Element {
        ...
        xmlSigningTime.textContent = XmlJs.Convert.FromDateTime(this.SigningTime);
        ...
        if (this.signatureProductionPlace && this.signatureProductionPlace.HasChanged()) {
