...
export class UnsignedSignatureProperties extends XmlXadesObject {
    ...
    protected name = XmlXades.ElementNames.UnsignedSignatureProperties;
    ...
    public LoadXml(element: Element, counterSignedXmlElement?: Element): void {
        ...
        let xmlNodeList = this.GetChildren(XmlXades.ElementNames.CounterSignature);
        for (let xmlNode of xmlNodeList) {
            if (xmlNode) {
                ...
                let counterSignatureElement: Element | null = null;
                for (let i = 0; i < xmlNode.childNodes.length && !counterSignatureElement; i++) {
                    if (xmlNode.childNodes[i].nodeType === XmlJs.XmlNodeType.Element) {
                        counterSignatureElement = xmlNode.childNodes[i] as Element;
                    }
                }
                if (counterSignatureElement) {
                    newXadesSignedXml.LoadXml(counterSignatureElement);
                    this.CounterSignatureCollection.Add(newXadesSignedXml);
                }
                else {
                    throw new XmlJs.XmlError(XmlJs.XE.CRYPTOGRAPHIC, "CounterSignature element does not contain signature");
                }
            }
        }

        this.SignatureTimeStampCollection.Clear();
        xmlNodeList = this.GetChildren(XmlXades.ElementNames.SignatureTimeStamp);
        for (let xmlNode of xmlNodeList) {
        ...
        let xmlCompleteCertificateRefs = this.GetElement(XmlXades.ElementNames.CompleteCertificateRefs, false);
        ...
        let xmlCompleteRevocationRefs = this.GetElement(XmlXades.ElementNames.CompleteRevocationRefs, false);
        ...
        xmlNodeList = this.GetChildren(XmlXades.ElementNames.SigAndRefsTimeStamp);
        if (xmlNodeList.length) {
            this.RefsOnlyTimeStampFlag = false;
            for (let xmlNode of xmlNodeList) {
                let newTimeStamp = new XadesTimeStamp(XmlXades.ElementNames.SigAndRefsTimeStamp);
                newTimeStamp.LoadXml(xmlNode);
                this.SigAndRefsTimeStampCollection.Add(newTimeStamp);
            }
        }
        else {
            xmlNodeList = this.GetChildren(XmlXades.ElementNames.RefsOnlyTimeStamp);
            if (xmlNodeList.length) {
        ...
        let xmlCertificateValues = this.GetElement(XmlXades.ElementNames.CertificateValues, false);
        ...
        let xmlRevocationValues = this.GetElement(XmlXades.ElementNames.RevocationValues, false);
        ...
        xmlNodeList = this.GetChildren(XmlXades.ElementNames.ArchiveTimeStamp);
        for (let xmlNode of xmlNodeList) {
