...
import { NoticeRef } from "./notice_ref";
...
export class SPUserNotice extends SigPolicyQualifier {
    ...
    public LoadXml(element: Element): void {
        ...
        let xmlSPUserNotice = this.GetElement(XmlXades.ElementNames.SPUserNotice, true);

        let xmlNoticeRef = SPUserNotice.GetElement(xmlSPUserNotice, XmlXades.ElementNames.NoticeRef, false);
        if (xmlNoticeRef) {
            this.NoticeRef = new NoticeRef();
            this.NoticeRef.LoadXml(xmlNoticeRef);
        }

        let xmlExplicitText = SPUserNotice.GetElement(xmlSPUserNotice, XmlXades.ElementNames.ExplicitText, false);
        if (xmlExplicitText && xmlExplicitText.textContent) {
            this.ExplicitText = xmlExplicitText.textContent;
        }

        if (!(xmlNoticeRef || xmlExplicitText))
            throw new XmlJs.XmlError(XmlJs.XE.CRYPTOGRAPHIC, "Missing NoticeRef or ExplicitText element");
    }
